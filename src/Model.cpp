...
void Model::loadMesh(unsigned int indMesh)
{
	...
	if (JSON["meshes"][indMesh]["primitives"][0].contains("material"))
	{
		unsigned int materialIndex =
			JSON["meshes"][indMesh]["primitives"][0]["material"];

		auto material = JSON["materials"][materialIndex];

		if (material.contains("pbrMetallicRoughness") &&
			material["pbrMetallicRoughness"].contains("baseColorFactor"))
		{
			auto baseColor =
				material["pbrMetallicRoughness"]["baseColorFactor"];

			mesh.baseColorFactor = glm::vec4(
				baseColor[0],
				baseColor[1],
				baseColor[2],
				baseColor[3]
...
std::vector<Texture> Model::getTextures()
{
	...
	for (unsigned int i = 0; i < JSON["images"].size(); i++)
	{
		...
		if (!skip)
		{
			...
			{
				Texture diffuse = Texture((fileDirectory + texPath).c_str(), "diffuse", loadedTex.size(), GL_RGBA,GL_UNSIGNED_BYTE);
			...
			{
				Texture specular = Texture((fileDirectory + texPath).c_str(), "specular", loadedTex.size(),GL_RGBA,GL_UNSIGNED_BYTE);
